...
A unified infrastructure to discover, secure, and execute Model Context Protocol (MCP) tools. Bridges local processes (STDIO) to remote APIs (SSE/HTTP) with built-in Sandbox and OAuth 2.1 support.
...
- **Protocol Bridge** - Instantly expose local STDIO-based MCP servers as remote StreamableHTTP (SSE) endpoints.
...
- **Protocol Adaptation** - Connect remote agents (via HTTP/SSE) to local CLI tools (via STDIO).
...
- **Unified Gateway** - REST API to query and execute tools remotely
...
### Path A: Deploy Enterprise Gateway (Recommended)
...
### Path B: Use as Registry SDK (Data Only)
